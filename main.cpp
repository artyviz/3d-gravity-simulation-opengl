...
int main() {
    ...
    while (!glfwWindowShouldClose(window) && running) {
        ...
        ImGui::NewFrame();
        ImGui::Begin("Space Simulation Controls");
        ...
        ImGui::SliderFloat("Simulation Speed", &deltaTimeMultiplier, 0.1f, 2.0f);
        ImGui::Text("Frame Time: %.3f ms", deltaTime * 1000.0f);
        ImGui::Text("Objects: %zu", objs.size());
        ImGui::Text("Current Mass: %.2e", objs.empty() ? 0.0 : objs.back().mass);
        ImGui::End();
        ImGui::Render();
